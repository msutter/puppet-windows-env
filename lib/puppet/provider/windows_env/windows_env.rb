...
Puppet::Type.type(:windows_env).provide(:windows_env) do
  ...
  if Puppet.features.windows_env?
    module self::WinAPI
      ...
      ffi_lib :User32
      attach_function :SendMessageTimeout, :SendMessageTimeoutA, [:long, :uint, :long, :pointer, :uint, :uint, :ulong], :long

      ffi_lib :Advapi32
      attach_function :RegLoadKey, :RegLoadKeyA, [:ulong, :pointer, :pointer], :long

      attach_function :RegUnLoadKey, :RegUnLoadKeyA, [:ulong, :pointer], :long
    end
  end
  ...
  def name_to_sid(name)
    ...
    if Puppet::Util::Windows::Security.respond_to?(:name_to_sid)
  ...
  def broadcast_changes
    ...
    self.class::WinAPI.SendMessageTimeout(_HWND_BROADCAST, _WM_SETTINGCHANGE, 0, 'Environment', 2, @resource[:broadcast_timeout], 0)
  end    
